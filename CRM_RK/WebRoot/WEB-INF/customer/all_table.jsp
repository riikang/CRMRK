...
<html>
	...
	function confirm_delete(){
		...
		if(rows.length>=1){
			if(!confirm("确认删除？")){
	        	...
	        	$.ajax({
					type: "POST",
					url: "crm/customerPAction_ifcandelete.action",
					data: {deleteid:deleteid},
					dataType: 'text',
					success: function(result) {
						if (result=="1") {
							alert("所选客户存在关联数据，请删相关销售机会、订单和服务记录后，再进行此操作");
							return 
						}else{
							window.location.href='<%=path %>/crm/customerPAction_deleteSomecustomerp.action?ccid='+deleteid;
							alert("删除成功");
						}
					}
				});
  ...
  <body>
		...
		$(function () {
			$('#reportTable').bootstrapTable({
				...
				columns: [{field:"checkbox",checkbox: true,align:"center",valign:"middle"},
						  {field:"id",title:"客户编号",align:"center",valign:"middle",sortable:"true"},
	...
	<s:iterator value="customerPs" id="cps">
		<script>
		init.dates[ii] = {
				//<input type="checkbox" onchange="confirm_checked(this)" id="ccid" name="ccid" value="<s:property value="#cps.id"/>"/>&nbsp;
				'id' : '<s:property value="#cps.id"/>',
